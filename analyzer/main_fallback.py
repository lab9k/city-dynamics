...
def run_imports_fallback():
    ...
    brt = process_fallback.Process_buurtcombinatie(dbconfig)
    vbi = process_fallback.Process_verblijversindex(dbconfig)
    gvb_st = process_fallback.Process_gvb_stad(dbconfig)
    gvb_bc = process_fallback.Process_gvb_buurt(dbconfig)
    # tel = process_fallback.Process_tellus(dbconfig)
    alp_hist = process_fallback.Process_alpha_historical(dbconfig)
    alp_live = process_fallback.Process_alpha_live(dbconfig)
...
def linear_model_fallback(drukte):
    ...
    drukte['gvb'] = process_fallback.norm(drukte.gvb)
...
##############################################################################
def write_to_db_fallback(drukte):
    """Write data to database."""
    log.debug('Writing data to database.')
    dbconfig = args.dbConfig[0]
    connection = process_fallback.connect_database(dbconfig)
    drukte.to_sql(
...
def run_fallback():
    ...
    drukte = run_imports_fallback()
    drukte = linear_model_fallback(drukte)
    # drukte = pipeline_model(drukte)
    write_to_db_fallback(drukte)
