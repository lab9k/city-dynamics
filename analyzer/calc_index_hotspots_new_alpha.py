...
def main():
    ...
    join_hotspots_query = """
      DROP TABLE IF EXISTS alpha_locations_expected_hotspots;
      create
      table
        alpha_locations_expected_hotspots as with hs as(
          select
            *
          from
            "hotspots"
          ...
          hs."Hotspot",
          alpha_locations_expected.*
        from
          alpha_locations_expected join hs on
          st_intersects(
    ...
    google_hotspots = pd.read_sql(sql="SELECT * FROM alpha_locations_expected_hotspots", con=conn)
    ...
    google_week_hotspots.rename(columns={'expected': 'alpha_week'}, inplace=True)
    ...
    drukteindex_hotspots.to_sql(name='drukteindex_hotspots_new_alpha', con=conn, if_exists='replace')
